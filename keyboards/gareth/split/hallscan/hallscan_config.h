...
// Output all sensor readings once a second
// Dont' forget to switch this off when you've done with it
// #define VERBOSE_DEBUG

// The default is to detect a keypress when the reading drops.
// Define SENSOR_POSITIVE to detect a keypress when it rises
...
typedef enum sensor_names {
    S_Q_J = 1, S_W_L,     S_F_U,      S_P_Y,  S_G_APOS,
    S_A_H,     S_R_N,     S_S_E,      S_T_I,     S_D_O,
    S_Z_K,     S_X_M,  S_C_COMM,    S_V_DOT, S_B_SLASH,
                     S_ESC_BSPC,  S_SPC_ENT, S_TAB_DEL,
